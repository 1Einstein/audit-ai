...
export default async function AuditPage({
  ...
  if (targetUser) {
    try {
      // Using the v2 endpoint which is the most current for March 2026
      const res = await fetch(`https://api.scrapecreators.com/v2/instagram/user/full?handle=${targetUser}`, {
        ...
        cache: 'no-store' // FORCES the "Analyze" button to get new data
      });
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white font-sans">
      ...
      <main className="relative z-10 max-w-5xl mx-auto pt-20 px-6">
        ...
        {/* Updated Form to ensure it SUBMITS on click */}
        <form action="/" method="GET" className="max-w-2xl mx-auto mb-20">
          <div className="flex bg-zinc-900/50 border border-zinc-800 rounded-2xl p-2 backdrop-blur-xl focus-within:border-green-500/50 transition-all">
            ...
            />
            {/* type="submit" is the key to making the button work */}
        ...
        {targetUser ? (
          ...
          <div className="text-center text-zinc-800 font-bold uppercase tracking-widest text-xs">
            Ready for Input
          </div>
        )}
      </main>
