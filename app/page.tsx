...
export default async function AuditPage({
  ...
  if (targetUser) {
    try {
      // Changed to the v1 endpoint with 'username' parameter to fix 404
      const res = await fetch(`https://api.scrapecreators.com/v1/instagram/info?username=${targetUser}`, {
        ...
        cache: 'no-store' // Forces a fresh pull when you click Analyze
      });
      ...
      <main className="relative z-10 max-w-5xl mx-auto pt-20 px-6">
        ...
        <form action="/" method="GET" className="max-w-2xl mx-auto mb-20">
          <div className="flex bg-zinc-900/50 border border-zinc-800 rounded-2xl p-2 backdrop-blur-xl focus-within:border-green-500/50 transition-all">
            ...
            />
            <button type="submit" className="bg-white text-black font-bold px-10 rounded-xl hover:bg-zinc-200 transition-all">
              Analyze
            </button>
          </div>
        </form>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in duration-700">
            <div className="md:col-span-2 bg-zinc-900/40 border border-zinc-800 p-8 rounded-[32px]">
              <div className="flex items-center gap-2 mb-6 text-zinc-500">
                ...
                <span className="text-[10px] font-bold uppercase tracking-[0.2em]">Audit Logs // {targetUser}</span>
              </div>
              <pre className="text-[11px] font-mono text-green-400/90 overflow-auto max-h-[400px]">
